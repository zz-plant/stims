...
describe('Workspace shell toast regression', () => {
  test('shows the lighter-mode warning only once per session', () => {
    ...
    expect(toastHookSource).toMatch(
      /useEffect\(\(\) => \{[\s\S]*?engineSnapshot\.backend !== 'webgl'[\s\S]*?webglWarningShownRef\.current = true;[\s\S]*?setToast\(\{\s*message: 'Using a lighter graphics mode so playback stays smooth\.',\s*tone: 'warn',\s*\}\);[\s\S]*?window\.setTimeout\([\s\S]*?4200\);\s*\}, \[\s*engineSnapshot\?\.backend,\s*engineSnapshot\?\.runtimeReady,?\s*\]\);/u,
    );
  });
