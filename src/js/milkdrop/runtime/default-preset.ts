/**
 * The first-run preset, compiled into the bundle so the very first frame is the
 * preset we actually chose for that moment — not a placeholder.
 *
 * This used to be a hand-written stand-in ('Signal Bloom') shown for the few
 * hundred milliseconds before `catalog.json` (1.6MB, ~75KB gzipped) lands. That
 * meant a first-time visitor's opening frames were a preset nobody picked,
 * followed by a crossfade to the real one, and it forced startup selection to
 * special-case an id that was not in the catalog.
 *
