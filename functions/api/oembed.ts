...
export async function onRequest(context: EventContext): Promise<Response> {
  ...
  if (!presetId && targetUrl.pathname.startsWith('/preset/')) {
    presetId = targetUrl.pathname.slice('/preset/'.length).split('/')[0] || null;
  }
  ...
  const embedWidth = Math.min(
    1280,
    Math.max(320, parseInt(requestUrl.searchParams.get('maxwidth') || '800', 10)),
  );
  const embedHeight = Math.min(
    720,
    Math.max(240, parseInt(requestUrl.searchParams.get('maxheight') || '450', 10)),
  );
