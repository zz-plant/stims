export async function onRequest(context) {
    ...
    try {
        const { description, count = 3 } = (await request.json());
        ...
        if (env.AI) {
            ...
            results.push(...responses.map((r) => cleanMilkSource(r.response)));
        }
        return json({ presets: results });
    }
    catch (error) {
        return json({ error: error instanceof Error ? error.message : 'Batch failed' }, 500);
    }
}
function cleanMilkSource(raw) {
    let s = raw.replace(/```[\w]*\n?/g, '').trim();
    if (!s.includes('[preset00]'))
        s = `[preset00]\n${s}`;
    return s;
}
