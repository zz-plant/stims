...
export async function onRequest(context) {
    ...
    try {
        ...
        if (method === 'POST' && pathParts.length === 0) {
            ...
            await env.DB.prepare(`INSERT INTO presets (id, title, author, tags, rating, downloads, created_at)
         VALUES (?, ?, ?, ?, 0, 0, datetime('now'))`)
                .bind(id, body.title, body.author || 'Anonymous', (body.tags || []).join(','))
                .run();
        ...
        if (method === 'POST' &&
            ...
            const existing = await env.DB.prepare('SELECT id FROM favorites WHERE preset_id = ? AND session_id = ?')
                .bind(id, sessionId)
                .first();
            if (existing) {
                await env.DB.prepare('DELETE FROM favorites WHERE preset_id = ? AND session_id = ?')
                    .bind(id, sessionId)
                    .run();
                await env.DB.prepare("UPDATE presets SET rating = MAX(rating - 1, 0), updated_at = datetime('now') WHERE id = ?")
                    .bind(id)
                    .run();
                return json({ favorited: false });
            }
            await env.DB.prepare('INSERT INTO favorites (preset_id, session_id) VALUES (?, ?)')
                .bind(id, sessionId)
                .run();
            await env.DB.prepare("UPDATE presets SET rating = rating + 1, updated_at = datetime('now') WHERE id = ?")
                .bind(id)
                .run();
        ...
        if (method === 'GET' && pathParts.length === 1) {
            const id = pathParts[0].replace('community:', '');
            const preset = await env.DB.prepare('SELECT id, title, author, tags, rating, downloads, created_at FROM presets WHERE id = ?')
                .bind(id)
                .first();
            if (!preset) {
...
function getSessionId(request) {
    const raw = request.headers.get('x-stims-session-id') ||
