    ...
    try {
        const result = await ai.run('@cf/ibm-granite/granite-4.0-h-micro', {
            messages: [
                {
                    role: 'system',
                    content: 'Classify this request. Output ONLY JSON: {"complexity":"simple|moderate|complex","needsReasoning":true|false}. Consider: under 6 words = simple, 6-15 = moderate, 15+ with technical terms = complex. Requests involving math, physics, patterns, multiple interactions = needsReasoning.',
                },
    ...
    try {
        ...
        if (env.DB && env.AI) {
            try {
                ...
                if (queryEmbedding) {
                    const { results } = await env.DB.prepare('SELECT preset_id, embedding FROM preset_embeddings').all();
        ...
        const systemPrompt = buildGeneratePrompt(body.description, body.complexity || 'moderate');
            ...
            try {
                const nameResult = await env.AI.run('@cf/ibm-granite/granite-4.0-h-micro', {
                    messages: [
                        {
                            role: 'system',
                            content: 'Generate a short, evocative title (3-6 words) for this MilkDrop visualizer preset. Be creative. Output only the title.',
                        },
