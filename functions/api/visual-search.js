...
const EMBEDDING_CACHE_TTL_MS = 60000;
const MAX_EMBEDDINGS_SCAN = 5000;
async function loadEmbeddingCache(env) {
    if (embeddingCache &&
        Date.now() - embeddingCacheTimestamp < EMBEDDING_CACHE_TTL_MS) {
        return embeddingCache;
    }
    const { results } = await env.DB.prepare('SELECT preset_id, embedding FROM preset_embeddings LIMIT ?1')
        .bind(MAX_EMBEDDINGS_SCAN)
        .all();
    embeddingCache = results.map((row) => {
        const vector = new Float32Array(JSON.parse(row.embedding));
        let norm = 0;
