...
export function applyAudioRotation(object, audioData, rotationSpeed, band = 'average') {
    let avgFrequency;
...
export function applyAudioScale(object, audioData, scaleFactor, band = 'average') {
    let avgFrequency;
...
function getLowFrequency(audioData) {
    return audioData.slice(0, audioData.length * 0.33).reduce((acc, val) => acc + val, 0) / (audioData.length * 0.33);
}

function getMidFrequency(audioData) {
    return audioData.slice(audioData.length * 0.33, audioData.length * 0.66).reduce((acc, val) => acc + val, 0) / (audioData.length * 0.33);
}

function getHighFrequency(audioData) {
    return audioData.slice(audioData.length * 0.66).reduce((acc, val) => acc + val, 0) / (audioData.length * 0.33);
}
