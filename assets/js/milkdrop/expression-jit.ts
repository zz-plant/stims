...
const OPERATORS: Record<string, string> = {
  '+': 'l+r', '-': 'l-r', '*': 'l*r', '/': '(r===0?0:l/r)',
  '%': '(R===0?0:l%R)',
  '^': 'l**r',
  '<': '(l<r?1:0)', '<=': '(l<=r?1:0)', '>': '(l>r?1:0)', '>=': '(l>=r?1:0)',
  '==': '(l===r?1:0)', '!=': '(l!==r?1:0)',
  '&&': '(l&&r?1:0)', '||': '(l||r?1:0)',
};
...
const FUNCTIONS: Record<string, string> = {
  ...
  abs: 'Math.abs', sqrt: 'Math.sqrt(Math.max(0,a))',
  pow: '(a**b)', exp: 'Math.exp', log: 'Math.log(Math.max(1e-6,a))',
  sqr: '(a*a)', floor: 'Math.floor', ceil: 'Math.ceil',
  int: '((a|0)===a?a:Math.trunc(a))',
  frac: '(a-Math.floor(a))', sign: 'Math.sign',
  min: '(a<b?a:b)', max: '(a>b?a:b)',
  clamp: '(Math.min(Math.max(a,b),c))',
  mix: '(a+(b-a)*c)', lerp: '(a+(b-a)*c)',
  step: '(a>b?1:0)',
  smoothstep: '(function(e0,e1,x){var t=Math.max(0,Math.min((x-e0)/(e1-e0),1));return t*t*(3-2*t)})(a,b,c)',
  sigmoid: '(1/(1+Math.exp(-a*(b||1))))',
  above: '(a>b?1:0)', below: '(a<b?1:0)', equal: '(a===b?1:0)',
  if: '(a!==0?b:c)',
  mod: '(b===0?0:a%b)', fmod: '(b===0?0:a%b)',
  bor: '((a|0)|(b|0))', band: '((a|0)&(b|0))', bnot: '(~(a|0))',
  rand: 'r()*(a||1)',
};
...
export function evaluateJit(
  ...
  if (!fn) {
    ...
    fn = new Function('e', 'r', `"use strict";return (${body});`) as unknown as JitFn;
    rawCache.set(key, fn);
  }
