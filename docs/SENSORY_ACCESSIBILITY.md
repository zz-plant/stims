...
- `scripts/flash-analysis.ts` + `scripts/analyze-preset-flash.ts` (`bun run lab:flash-audit`) — **the tool to use for any WCAG question**, corpus-scale that implements the *real* WCAG 2.3.1 general-flash definition directly: `FLASH_LUMINANCE_DELTA = 0.1`, `FLASH_DARKER_CEILING = 0.8`, `FLASHES_PER_SECOND_LIMIT = 3`, and a sliding-window area test at `VISUAL_FIELD_FRACTION = 1/3` — matching the [specification below](#flash-safety-specification) exactly, and unit-tested against synthetic timelines (`tests/unit/flash-analysis.test.ts`) independent of a live GPU. It already supports the stratified-sampling approach this doc recommends (`--count=N`, `--ids=a,b,c`, `--all`).
  ...
  **One caveat before this is treated as a safety result.** The margin is moderate, not comfortable — 14–16% per direction against a 25% threshold — so a higher-energy stimulus could plausibly push a directional component over; every measurement above used the synthetic preview waveform with 2 Hz beat pulses, not real bass-heavy music. Re-run against real audio before publishing a prevalence figure or shipping a safety claim.

  The **red flash** criterion (saturated-red transitions, a separate WCAG formula) is now implemented in `flash-analysis.ts` (`redFlashValue`, `isSaturatedRed`, `isRedFlashTransition`) and wired through `analyze-preset-flash.ts`, unit-tested including the case a general-flash-only measure misses (a black↔red strobe under the luminance delta). It has not yet been run at corpus scale — that run, and the same real-audio caveat above, both apply before red-dominant presets can be called measured.
