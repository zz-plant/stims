...
Everything above is standard MilkDrop, portable to Winamp, projectM, and Butterchurn. Stims also feeds presets interaction and device-motion signals — `inputX`/`inputY`, `gestureScale`, `gestureRotation`, `motionX`/`motionY`/`motionZ`, and their aliases — documented in the [signal contract](../MILKDROP_PRESET_RUNTIME.md). **These are not standard MilkDrop.** A preset that reads them will still compile and run fine elsewhere (unknown identifiers just evaluate to 0), but the interactivity silently disappears. Use them freely for Stims-native work; if portability matters, keep them behind a variable you can zero out, or skip them.
