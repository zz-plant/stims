...
| **Hero** | Copy is inline in `index.html`. Primary CTA links to `/milkdrop/?audio=demo`. Secondary CTA links to `/milkdrop/`. Helper links may deep-link to `/milkdrop/?panel=browse` and `/milkdrop/?panel=editor`. | Hero makes the route split explicit: home explains, `/milkdrop/` launches. Supporting cards summarize first-run expectations and route choices. | CTA focus treatment stays visible. Modified clicks should fall back to native navigation. |
