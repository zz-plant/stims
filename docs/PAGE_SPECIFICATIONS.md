...
| --- | --- | --- | --- |
| **Global frame** | Loads shared styles from `assets/css/base.css` and `assets/css/index.css`. Uses `lang="en"` and preserves the saved theme before paint. | Content flows as nav → hero → experience framing → preset showcase → lineage → footer. | Theme toggle must keep `aria-pressed` and `aria-label` in sync. Copy remains readable in light and dark themes. |
...
| **Hero** | Copy is inline in `index.html`. Primary CTA links to `/milkdrop/?audio=demo`. Secondary CTA links to `/milkdrop/`. A static launch summary reinforces that demo audio is the fastest path, while a single aside explains that deeper controls live inside `/milkdrop/` after launch. | Hero makes the route split explicit: home explains, `/milkdrop/` launches. The simplified layout keeps the CTA pair dominant, followed by a checklist-style summary and one contextual aside instead of route-choice cards or helper deep-links. | CTA focus treatment stays visible. Modified clicks should fall back to native navigation. |
