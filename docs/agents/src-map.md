...
| Services | `services/webmidi-controller.ts`, `services/webxr-service.ts`, `services/picture-in-picture-service.ts`, `services/microphone-permission-service.ts`, `services/optional-api.ts`, `services/render-service.ts`, `services/visual-embedding.ts`, `services/audio-service.ts`, `services/audio-matcher.ts` | `webmidi-controller.test.ts`, `webxr-service.test.ts`, `microphone-permission-service.test.ts`, `optional-api.test.ts`, `render-service-prewarm.test.ts`, `render-service-webxr-lifecycle.test.ts`, `visual-embedding.test.ts`, `services-pool.test.ts` | `check:quick` |
